...
function renderDownloads(downloads) {
  ...
  if (downloads.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" class="text-center">没有下载项目</td></tr>`;
    return;
  }
...
// 显示下载项目模态框
async function showDownloadModal(download = null) {
  ...
  if (download) {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/downloads/page/${download.id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      if (response.ok) {
        ...
        document.getElementById('download-page-id').value = detail.page_id || '';
        document.getElementById('download-content').value = detail.content || '';
      }
    } catch (error) {
      console.error('加载详情页面错误:', error);
      document.getElementById('download-page-id').value = '';
      document.getElementById('download-content').value = '';
    }
  } else {
