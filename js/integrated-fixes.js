...
(function(global) {
    ...
    messages.forEach(msg => {
      ...
      if (msg.message_type === 'emoji' || 
          (typeof msg.content === 'string' && 
           (msg.content.includes('emoji_path') || msg.content.includes('emoji_id')))) {
        try {
          let emojiData;
          ...
          if (emojiData && emojiData.emoji_path) {
            ...
            messageContent = `<img src="${emojiPath}" style="max-width: 120px; max-height: 120px; vertical-align: middle; border-radius: 8px;" alt="表情">`;
          } else {
  ...
  function createFollowingEmojiPicker(chatContainer) {
    ...
    picker.style.boxShadow = '0 4px 24px rgba(0,0,0,0.15)';
  ...
  function renderEmojiGrid(emojis) {
    ...
    emojis.forEach(emoji => {
      ...
      item.innerHTML = `
        <img src="${API_BASE_URL}${emoji.file_path}" alt="${emoji.emoji_name || emoji.file_name}" style="width: 32px; height: 32px; object-fit: contain;">
        <span class="emoji-item-name" style="display: none;">${emoji.emoji_name || emoji.file_name}</span>
      `;
  ...
  function sendEmoji(emoji) {
    ...
    if (currentEmojiPicker) {
      ...
      currentEmojiPicker.style.display = 'none';
    }
  ...
  if (window.MessageCenter) {
    window.MessageCenter.renderChatMessages = function(messages) {
  ...
  window.updateChatEmojiTabs = updateChatEmojiTabs;
  window.escapeHtml = escapeHtml;
  window.formatTime = formatTime;

  console.log('Integrated emoji and message fixes loaded successfully');
