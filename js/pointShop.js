...
(function() {
        ...
        <div class="admin-table-container">
          <table class="admin-table">
            ...
            <tbody id="items-tbody">
              <tr>
                <td colspan="${isPointShop ? 8 : 7}" class="loading-cell">
                  <i class="fas fa-spinner fa-spin"></i> 加载中...
                </td>
              </tr>
            </tbody>
  ...
  function renderAdminItems(items, shopType) {
    ...
    if (items.length === 0) {
      tbody.innerHTML = `
        <tr>
          <td colspan="${isPointShop ? 8 : 7}" class="empty-cell">暂无商品</td>
        </tr>
      `;
      return;
    }
  ...
  window.saveItem = async function(itemId, shopType) {
    ...
    if (shopType === 'points') {
      data.item_type = formData.get('item_type');
      if (data.item_type === 'upgrade') {
        data.upgrade_rank = parseInt(formData.get('upgrade_rank'));
      }
    }
